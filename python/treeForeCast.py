...
def draw(dataSet, tree, title, loc='left', color ='red',drawEnd=True):
    ...
    plt.plot(dataSet[:,0], yHat, linewidth=2.0, color=color)
    if drawEnd:
        plt.show()
...
#    df = ts.get_k_data(code = '002230', start = '2017-01-01') # 科大讯飞今年的股票数据
    ...
    e['close'] = df['fclose'] # 用收盘价作为分类标准Y轴, 以Y轴高低划分X成段，并分段拟合
    arr = np.array(e)
    tree = createTree(np.mat(arr), 100, 10)
    end = time.time()
    plt.figure(figsize=[20,11]) # 改变画布大小
    draw(arr, tree, stCode + ' start: ' + stDate + " TotalTime:" + str(end-start) + " " + str((end-start)/len(df)), drawEnd= False)
    e['close'] = df['close'] # 用收盘价作为分类标准Y轴, 以Y轴高低划分X成段，并分段拟合
    arr = np.array(e)
    tree = createTree(np.mat(arr), 100, 10)
    end = time.time()
    draw(arr, tree, stCode + ' start: ' + stDate + " TotalTime:" + str(end-start) + " " + str((end-start)/len(df)), color = 'blue')
