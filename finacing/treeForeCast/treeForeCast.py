...
def saveImg(plt, filename='', fileformat='png'):
    if not (len(filename) == 6):
        plt.savefig(filename)
    else:
        filename = os.path.join(tempfile.gettempdir(), filename)
        plt.savefig(filename, fileformat=fileformat)
...
def get_data(code=None, start='', end='',
    ...
    del dg['date']
    del dg['volume']
    dg.rename(columns={'open': 'fopen', 'close': 'fclose', 'high': 'fhigh',
                       'low': 'flow'}, inplace=True)
...
if __name__ == '__main__':
    ...
    fig = plt.figure(figsize=[20, 11])  # 改变画布大小
    plt.xlim([0, len(df) + 1])
    plt.ylim([min(df['fclose']) * 0.97, max(df['fclose']) * 1.03])
    i = 0;
    sl = 800
    for c in mp.colors.cnames:
        ...
        draw(arr, tree, stCode + ' start: ' + stDate + " TotalTime:" + str(
            end - start) + " " + str((end - start) / len(df)) + " " + str(
            len(e)), color=c)
        # i+=1
        # 显示延迟
        # if end-start2 < 5:
        #     # time.sleep(5+end-start2)
        #     time.sleep(end-start2)
    ...
    # plt.savefig('{}.png'.format(stCode), dpi=fig.dpi)
