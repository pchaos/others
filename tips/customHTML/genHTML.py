...
-------------------------------------------------
   File Name：  genHTML
   Last Modified:   2024-10-22 00:23:18
...
-------------------------------------------------
   Change Activity:
                   2024-10-20:增加每行分隔成几列（numPerLine）,在运行时可以改变该值，影响最后生成的文件
-------------------------------------------------
"""
__author__ = 'pchaos'
...
class genHTML:
    ...
    @property
    def numPerLine(self):
        """The number Per Line : 每行分隔成几列"""
        return self.__numPerLine

    @numPerLine.setter
    def numPerLine(self, value):
        self.__numPerLine = value

    def getRealFileName(self, filename):
        fx1 = os.path.exists(os.path.join(os.path.realpath("./"), filename))
        fx2 = os.path.exists(os.path.join(os.path.realpath("./customHTML"), filename))
        if (not fx1 and not fx2) or len(filename) == 0:
            print("文件不存在：{}".format(os.path.join(os.path.realpath("./"), filename)))
            return None
        # 读取
        if fx1:
    ...
    def getSource(self, iniFile=""):
        ...
        for a in alist:
            try:
                ...
                # 读取空行 自动填充空数据至数据个数能被self.numPerLine整除
                for j in range(self.numPerLine):
                    if i % self.numPerLine > 0:
                        dlist.append(item)
                        i += 1
                    else:
                        break
        # 最终添加的元素长度排成五列
        item = {"caption": "", "href": ""}
        while len(dlist) % self.numPerLine > 0:
            dlist.append(item)
        return dlist

    def genHTML(self, outputFilename=None, title="", prettify=True, template="template.html", numPerLine=5):
        ...
        render = self.renders(dlist, prettify, template, title, numPerLine)
    ...
    def renders(self, dlist, prettify, template, title, numPerLine=5):
        ...
        render = template.render(
            title=title, sites=dlist, numPerLine=numPerLine
