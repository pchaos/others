...
安装V2ray:
	1.Socks5
	2.Shadowsocks(兼容原版客户端)
	3.Shadowsocks+Mkcp(不兼容原版客户端)
	4.TCP
	5.TCP+动态端口
	6.TCP+HTTP伪装(监听80端口)
	7.TCP+TLS(监听443端口)
	...
	11.Mkcp+微信视频流量伪装
	12.Mkcp+动态端口
	13.Mkcp+微信视频流量伪装+动态端口
	14.Websocket+TLS(使用Path分流，可使用CDN，监听443端口)
	15.Websocket+TLS+网站伪装(使用Caddy转发流量，使用Path分流，可使用CDN，监听443端口)
--------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------"
	...
	if [[ ${determine_type} = "" ]]; then
		...
		exit 1
	elif [[ ${determine_type} -gt 19 ]]; then
		clear
		echo -e "${error_font}请输入正确的序号！"
		exit 1
	else
...
function data_processing(){
	clear
	echo -e "正在处理请求中..."
	if [[ ${determine_type} = "16" ]]; then
		prevent_uninstall_check
		uninstall_program
	elif [[ ${determine_type} = "17" ]]; then
		prevent_uninstall_check
		start_service
	elif [[ ${determine_type} = "18" ]]; then
		prevent_uninstall_check
		stop_service
	elif [[ ${determine_type} = "19" ]]; then
		prevent_uninstall_check
		restart_service
	else
		...
		if [[ ${determine_type} = "1" ]]; then
			...
			wget -O "/etc/v2ray/config.json" "https://download.v2ray-install.ml/configs/shadowsocks.json"
			if [[ $? -eq 0 ]];then
				clear
				echo -e "${ok_font}SS 配置文件下载成功。"
			else
				clear
				echo -e "${error_font}SS 配置文件下载失败！"
				clear_install
				exit 1
			fi
			input_port
			...
			if [[ ${install_ss_password} = "" ]]; then
				...
				if [[ $? -eq 0 ]];then
					clear
					echo -e "${ok_font}SS连接密码配置成功。"
				else
					clear
					echo -e "${error_font}SS连接密码配置失败！"
					clear_install
					exit 1
				fi
			fi
			...
			wget -O "/etc/v2ray/config.json" "https://download.v2ray-install.ml/configs/shadowsocks-mkcp.json"
			if [[ $? -eq 0 ]];then
				clear
				echo -e "${ok_font}SS 配置文件下载成功。"
			else
			...
			wget -O "/etc/v2ray/config.json" "https://download.v2ray-install.ml/configs/tcp.json"
			clear
			input_port
			...
			wget -O "/etc/v2ray/config.json" "https://download.v2ray-install.ml/configs/mkcp.json"
			if [[ $? -eq 0 ]];then
				clear
				echo -e "${ok_font}V2Ray 配置文件下载成功。"
			else
			...
			wget -O "/etc/v2ray/config.json" "https://download.v2ray-install.ml/configs/mkcp-wechatvideo.json"
			if [[ $? -eq 0 ]];then
				clear
				echo -e "${ok_font}V2Ray 配置文件下载成功。"
			else
			...
			wget -O "/etc/v2ray/config.json" "https://download.v2ray-install.ml/configs/mkcp-dynport.json"
			if [[ $? -eq 0 ]];then
				clear
				echo -e "${ok_font}V2Ray 配置文件下载成功。"
			else
			...
			wget -O "/etc/v2ray/config.json" "https://download.v2ray-install.ml/configs/mkcp-wechatvideo-dynport.json"
			if [[ $? -eq 0 ]];then
				clear
				echo -e "${ok_font}V2Ray 配置文件下载成功。"
			else
			...
			wget -O "/etc/v2ray/config.json" "https://download.v2ray-install.ml/configs/websocket-tls-path.json"
			if [[ $? -eq 0 ]];then
				clear
				echo -e "${ok_font}V2Ray 配置文件下载成功。"
			else
...
function clear_install(){
	...
	elif [ "${determine_type}" -eq "15" ];then
		full_domain=$(cat /etc/v2ray/full_domain.txt)
		bash ~/.acme.sh/acme.sh --revoke -d ${full_domain} --ecc
		bash ~/.acme.sh/acme.sh --remove -d ${full_domain} --ecc
		rm -rf ~/.acme.sh
...
function close_port(){
	clear
	echo -e "正在设置防火墙中..."
	install_port=$(cat /etc/v2ray/install_port.txt)
	iptables-save > /etc/iptables.up.rules
	echo -e '#!/bin/bash\n/sbin/iptables-restore < /etc/iptables.up.rules' > /etc/network/if-pre-up.d/iptables
	chmod +x /etc/network/if-pre-up.d/iptables
	iptables -D INPUT -m state --state NEW -m tcp -p tcp --dport ${install_port} -j ACCEPT
	iptables -D INPUT -m state --state NEW -m udp -p udp --dport ${install_port} -j ACCEPT
	iptables-save > /etc/iptables.up.rules
	if [[ $? -eq 0 ]];then
		clear
...
function echo_v2ray_config(){
	if [[ ${determine_type} = "1" ]]; then
		...
		clear
		sslink="ss://"$(echo -e "${install_ss_encryption_type}:${install_ss_password}@${Address}:${install_port}" | base64)
		echo -e "您的连接信息如下："
		echo -e "地址(Hostname)：${Address}"
		echo -e "端口(Port)：${install_port}"
		echo -e "加密方式：${install_ss_encryption_type}"
		echo -e "连接密码：${install_ss_password}"
		echo -e "SS链接：${green_backgroundcolor}${sslink}${default_fontcolor}"
	elif [[ ${determine_type} = "3" ]]; then
		clear
		echo -e "您的连接信息如下："
		echo -e "地址(Hostname)：${Address}"
